...
# 
$main_package_json = JSON.parse(IO.read(path_to_package_json))
...
def packageJsonFor(language_extension, version:nil)
    ...
    # icon
    full_icon_path = svgPath(language_extension)
    if not File.file?(full_icon_path)
        createIconFor(language_extension, full_icon_path)
    end
    # create the png version
    system "node #{$path_to_convert_svg_program}"
    new_package_json["icon"] = iconPath(language_extension)
    # keywords
