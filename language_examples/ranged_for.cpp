...
static void prim_attrNames(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    state.mkList(v, args[0]->attrs->size()-2);
    
    const auto symbolTable = state.symbols;
    size_t n = 0;
    std::string staticId = "__id_static";
    std::string dynamicId = "__id_dynamic";
    for (nix::Attr & attr : *args[0]->attrs) {
        const auto symbol = attr.name;
        const auto name = std::string(symbolTable[symbol]);
        if (name == staticId || name == dynamicId) {
            continue;
        }
        (v.listElems()[n++] = state.allocValue())->mkString(state.symbols[attr.name]);
    }
...
static void prim_attrValues(EvalState & state, const PosIdx pos, Value * * args, Value & v)
{
    ...
    unsigned int n = 0;
    const auto symbolTable = state.symbols;
    const std::string staticId = "__id_static";
